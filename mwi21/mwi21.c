...
static void mwi21_html_page(uint8_t modelid, char *content, uint8_t mode) {
	...
	if (mode == 1) {
		...
		strcat(content,
			   "<TR class=\"thead\"><TH>&nbsp;</TH><TH>MIN</TH><TH>MID</TH><TH>MAX</TH><TH>MIN</TH><TH>MID</TH><TH>MAX</TH><TH>MIN</TH><TH>MID</TH><TH>MAX</TH><TH>MIN</TH><TH>MID</TH><TH>MAX</TH></TR>\n");
		lc = 0;
		for (n = 0; n < 16; n++) {
		...
		for (n = 0; n < 14; n++) {
			if (mwi_pid_names[modelid][n][0] != 0) {
				...
				strcat(content, tmp_str);
				sprintf(tmp_str, "<TD align=\"center\"><INPUT class=\"form-input\" onchange=\"check_value(%i,%i);\" id=\"%i-%i\" value=\"%0.1f\" type=\"text\"></TD>", n, 0, n, 0,
						(float)mwi_pid[modelid][n][0] / 10.0);
				strcat(content, tmp_str);
				sprintf(tmp_str, "<TD align=\"center\"><INPUT class=\"form-input\" onchange=\"check_value(%i,%i);\" id=\"%i-%i\" value=\"%0.3f\" type=\"text\"></TD>", n, 1, n, 1,
						(float)mwi_pid[modelid][n][1] / 1000.0);
				strcat(content, tmp_str);
