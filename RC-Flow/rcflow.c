...
void rcflow_calc_Embedded(void) {
	...
	for (link = 0; link < MAX_LINKS; link++) {
		if (RcData.RcLinkEmbedded[link].to[0] != 255 && RcData.RcLinkEmbedded[link].to[1] != 255) {
			RcData.RcPluginEmbedded[RcData.RcLinkEmbedded[link].to[0]].input[RcData.RcLinkEmbedded[link].to[1]].value +=
				RcData.RcPluginEmbedded[RcData.RcLinkEmbedded[link].from[0]].output[RcData.RcLinkEmbedded[link].from[1]].value;
			RcData.RcPluginEmbedded[RcData.RcLinkEmbedded[link].to[0]].input[RcData.RcLinkEmbedded[link].to[1]].value = rcflow_value_limit(
						RcData.RcPluginEmbedded[RcData.RcLinkEmbedded[link].to[0]].input[RcData.RcLinkEmbedded[link].to[1]].value, -1000, 1000);
		}
	}
	// Plugin-Calculation
	for (plugin = 0; plugin < MAX_PLUGINS; plugin++) {
		if (RcData.RcPluginEmbedded[plugin].type == RCFLOW_PLUGIN_INPUT) {
			...
			if (RcData.RcPluginEmbedded[plugin].input[4].value > 0) {
				RcData.RcPluginEmbedded[plugin].output[0].value = rcflow_value_limit((RcData.RcPluginEmbedded[plugin].input[0].value * RcData.RcPluginEmbedded[plugin].input[1].value / 1000) +
						(RcData.RcPluginEmbedded[plugin].input[2].value * RcData.RcPluginEmbedded[plugin].input[3].value / 1000), -1000, 1000);
			...
			if (RcData.RcPluginEmbedded[plugin].input[3].value > 0) {
				RcData.RcPluginEmbedded[plugin].output[0].value = rcflow_value_limit(RcData.RcPluginEmbedded[plugin].input[0].value, RcData.RcPluginEmbedded[plugin].input[2].value,
						RcData.RcPluginEmbedded[plugin].input[1].value);
			} else {
				RcData.RcPluginEmbedded[plugin].output[0].value = RcData.RcPluginEmbedded[plugin].input[0].value;
			}
			RcData.RcPluginEmbedded[plugin].output[1].value = RcData.RcPluginEmbedded[plugin].output[0].value * -1.0;
			...
			for (n = 0; n < 7; n++) {
				...
				if (RcData.RcPluginEmbedded[plugin].input[0].value >= range_begin && RcData.RcPluginEmbedded[plugin].input[0].value <= range_end && n < 6) {
					...
					RcData.RcPluginEmbedded[plugin].output[0].value = RcData.RcPluginEmbedded[plugin].input[n + 1].value + (int16_t)((float)(RcData.RcPluginEmbedded[plugin].input[n + 2].value -
							RcData.RcPluginEmbedded[plugin].input[n + 1].value) * range_pos);
					break;
				}
			}
			RcData.RcPluginEmbedded[plugin].output[0].value = rcflow_value_limit(RcData.RcPluginEmbedded[plugin].output[0].value, -1000, 1000);
			...
			if (RcData.RcPluginEmbedded[plugin].input[0].value >= RcData.RcPluginEmbedded[plugin].input[1].value &&
					RcData.RcPluginEmbedded[plugin].input[0].value <= RcData.RcPluginEmbedded[plugin].input[2].value) {
