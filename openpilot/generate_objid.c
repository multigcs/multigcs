	...
	while (cur != NULL) {
		if ((xmlStrcasecmp(cur->name, (const xmlChar *)"object")) == 0) {
			...
			if (settings == 1) {
				...
				fprintf(f_guimain,
						"			draw_text_button(esContext, \"%sShow\", VIEW_MODE_FCMENU, \"%s (%s)\", FONT_WHITE, -1.2, -0.9 + ny++ * 0.12, 0.002, 0.08, ALIGN_LEFT, ALIGN_TOP, UAVT_gui_show_num_change, (float)SNUM);\n",
