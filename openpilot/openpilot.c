...
SDL_Thread *sdl_thread_serial_openpilot = NULL;
uint8_t openpilot_thread_running = 1;
...
int thread_serial_openpilot (void *unused) {
	uint8_t openpilot_read_buf[256];
	uint8_t crc = 0;
	uint8_t res = 0;
	uint8_t n = 0;
	uint8_t n2 = 0;
//	uint8_t n3 = 0;
	...
	if (serial_fd_openpilot >= 0) {
		while (openpilot_thread_running == 1) {
			if (data_start == 0) {
			//	printf("-\n");
...
void openpilot_exit (void) {
	openpilot_thread_running = 0;
	if ( sdl_thread_serial_openpilot != NULL ) {
		SDL_WaitThread(sdl_thread_serial_openpilot, NULL);
		sdl_thread_serial_openpilot = NULL;
	}
