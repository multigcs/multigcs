...
void Android_JNI_SayText(const char *text) {
	SDL_Log("JNI_SayText");
	jmethodID mid;
	JNIEnv *mEnv = Android_JNI_GetEnv();
	mid = (*mEnv)->GetStaticMethodID(mEnv, mActivityClass, "SayText", "(Ljava/lang/String;)Z");
	if (mid) {
...
void Android_JNI_OpenLink(const char *text) {
	SDL_Log("OpenLink");
	jmethodID mid;
	JNIEnv *mEnv = Android_JNI_GetEnv();
...
void Android_JNI_ConnectSerial(char *name) {
	SDL_Log("JNI_ConnectSerial");
	jmethodID mid;
	JNIEnv *mEnv = Android_JNI_GetEnv();
	mid = (*mEnv)->GetStaticMethodID(mEnv, mActivityClass, "serialConnect", "(Ljava/lang/String;)Z");
	if (mid) {
...
int Android_JNI_ReadSerial(uint8_t *data, int len) {
	SDL_Log("JNI_ReadSerial");
	jmethodID mid;
	JNIEnv *mEnv = Android_JNI_GetEnv();
	mid = (*mEnv)->GetStaticMethodID(mEnv, mActivityClass, "serialRead", "(I)[B");
	if (mid) {
		jbyteArray jdat = (*mEnv)->CallStaticBooleanMethod(mEnv, mActivityClass, mid, (int)len);
	}
	return 0;
}
...
Sint64 Android_JNI_FileSeek(SDL_RWops *ctx, Sint64 offset, int whence) {
	if (ctx->hidden.androidio.assetFileDescriptorRef) {
		switch (whence) {
			case RW_SEEK_SET:
				if (ctx->hidden.androidio.size != -1 /* UNKNOWN_LENGTH */ && offset > ctx->hidden.androidio.size) {
