...
int thread_serial_gps (void *unused) {
	uint8_t n = 0;
	int res;
	char serial_buf[2];
	char line[1024];
	while (gui_running == 1 && gps_thread_running == 1) {
		while (serial_fd_gps >= 0 && (res = read(serial_fd_gps, serial_buf, 1)) > 0 && gui_running == 1 && gps_thread_running == 1) {
